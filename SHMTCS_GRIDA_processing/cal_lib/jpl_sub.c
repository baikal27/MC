...
#include <math.h>
#include <string.h>
#include "jpl_define.h"
//-----------------------------------------------------------------------------
static void constan(char nam[][6], double val[], double sss[], int *n);
static void state (double et2[],int list[],double pv[][6],double nut[]);
static void split (double tt, double fr[]);
static void interp(double buf[],double t[],int ncf,int ncm,int na,int ifl, double pv[]);
static void jtoc  (double jd, int date[], double *secs, int flag);
//-----------------------------------------------------------------------------
static FILE *F1;
static int KM=0,BARY=0;
static double PVSUN[6];
//-----------------------------------------------------------------------------

//-----------------------------------------------------------------------------
int init_jpl(char fname[])
//-----------------------------------------------------------------------------
{
 char nams[400][6];
 double ss[3], vals[400];
 int i,nvs;
 int npt=100;          /* result will be printed to screen every npt lines */

    if( (F1=fopen(fname,"rb"))==NULL) return -1;
...
//-----------------------------------------------------------------------------
{
 ...
 double emrat;
  ...
  fread(&R1.r1.ttl  , 252,1,F1);   // char   TTL[3][84]

//fread(&r1.cnam ,2400,1,F1);   // char   cnam[400][6]
  for(i=0;i<400;i++)
  for(j=0;j<  6;j++)
  fread(&R1.r1.cnam[i][j],1,1,F1); // char   cnam[400][6]
  
  fread(&R1.r1.ss   ,  24,1,F1);   // double sss[3]
  fread(&R1.r1.ncon ,   4,1,F1);   // int*4  ncon
  fread(&R1.r1.au   ,   8,1,F1);   // double au
  fread(&R1.r1.emrat,   8,1,F1);   // double emrat
  fread(&R1.r1.ipt  , 144,1,F1);   // int*4  ipt[12][3]
  fread(&R1.r1.numde,   4,1,F1);   // int*4  numde
  fread(&R1.r1.lpt  ,  12,1,F1);   // int*4  lpt[3]
  
  for(i=0;i<(RECSIZE-sizeof(rc1));i++)  fread(&ccc,1,1,F1);
  
  for(i=0;i<400;i++) fread(&cval[i],8,1,F1);
  for(i=0;i<(RECSIZE-3200);i++) fread(&ccc,1,1,F1);
...
****************************************************************************/
void constanold(char nam[][6], double val[], double sss[], int *n)
{
  int i,j;
...
**                                                                          **
*****************************************************************************/
static void interp(double coef[],double t[2],int ncf,int ncm,int na,int ifl,
            double posvel[6])
{
...
**                                                                          **
*****************************************************************************/
static void state(double et2[2],int list[12],double pv[][6],double nut[4])
{
  int i,j;
...
//------------------------------------------------------------------------------//
//	This subroutine converts julian day number to calendar date					//
//																				//
//	calling sequence parameters													//
//																				//
//	Input																		//
//		jd = julian date at which calendar date is wanted						//
//		flag = integer controlling calendar used in conversion					//
//																				//
//			-1 --> all dates expressed in julian calendar						//
//			+1 --> all dates expressed in gregorian calendar					//
//			 0 --> automatic mode, where dates before jd 2299160(OCT 4, 1582)	//
//				   are julian and later dates are gregorian						//
//																				//
//	Output																		//
//		date = 5-wd integer array with month, day, year, hour, minute			//
//			   of resulting astronomical calendar date							//
//		secs = D.P. seconds, including fraction; E.G., 51.206					//
//																				//
//	restrictions - input julian calendar must be for a positive					//
//			julian day number. for gregorian calendar							//
//			jd > 1721119 (dates must be A.D.)									//
//------------------------------------------------------------------------------//
static void jtoc(double jd, int date[], double *secs, int flag)
{
