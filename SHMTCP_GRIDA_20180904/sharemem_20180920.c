...
/*        
        ...
        if (((!bServerApp) || !(sharedMemory->serverReceiveBuffer[0])) &&
            ((bServerApp) || (clientIndex<0) || 
             !(sharedMemory->client[clientIndex].clientReceiveBuffer[0])))
            return 0;
        ...
        if (bServerApp)
             buffer = sharedMemory->serverReceiveBuffer;
        else buffer = sharedMemory->client[clientIndex].clientReceiveBuffer;
