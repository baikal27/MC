    ...
    if (sharedMemory)
    {
       	...
       	if (bServerApp) 
       	{
			...
			if (!alreadyExist)
           	...
           	success = 1;
			CmtReleaseLock(lock);
//			MessagePopup("Server Error","Success Shared Memory for Server.");
       	}
       	...
       	else 
       	{
			...
			clientIndex++;
           	success = 1;								 
			CmtReleaseLock(lock);
//			MessagePopup("Client Error","Success Shared Memory for Client.");
       	}
    }
	else
		MessagePopup("Shared Memory Error", "No Shared Memory.");
