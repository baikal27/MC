...
DWORD slew_stop2		 (char caxis, double lpos, double dvel); 
...
//-----------------------------------------------------------------------------
{
 ...
 long laxis;
    ...
    laxis=caxis-'A'+NODENO*2;
    code = AxmStatusReadMotion(laxis,&status);   // 지정축의 모션 구동 상태 값을 반환. 꽤나 복잡. manual 참조. 
    *dir = status >> 11 & 0x01;
...
//-----------------------------------------------------------------------------
{
 long laxis;
    ...
    laxis=caxis-'A'+NODENO*2;
    return AxmStatusReadInMotion (laxis,gate);    	   // 지정축의 모션이 완료되었으면(모션구동 없음. 0 반환), 모션구동중이면 1 반환.
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
DWORD go_const_test(char caxis, double velocity)
//-----------------------------------------------------------------------------
{
 DWORD mode;
 long laxis,iaxis;
...
//    AxmOverrideSetMaxVel(laxis,fabs(velocity)*1.1);
    ...
    read_gate(caxis,&mode);     
    if(mode==0) return AxmMoveVel(laxis,velocity,100000,100000);
	else if(mode==3) return 0;
...
//	return AxmMoveStartPos(laxis, dPos, dvel, VPROFILE[iaxis].acce, VPROFILE[iaxis].acce);
	return AxmMoveVel(laxis, dVel, VPROFILE[iaxis].acce, VPROFILE[iaxis].acce);
}
//-----------------------------------------------------------------------------
DWORD write_cpuoutput_bit(long  ibit, DWORD bit_value)
//-----------------------------------------------------------------------------------------------
