...
//  SW_LIMIT = 0;
    ...
    SetCtrlAttribute(tel_handle,TEL_IN ,ATTR_ON_COLOR ,VAL_RED);
    SetCtrlAttribute(tel_handle,TEL_IN ,ATTR_OFF_COLOR,VAL_GRAY);
    SetCtrlAttribute(tel_handle,TEL_OUT,ATTR_ON_COLOR ,VAL_RED);
    SetCtrlAttribute(tel_handle,TEL_OUT,ATTR_OFF_COLOR,VAL_PANEL_GRAY);
 ...
 char buff[100]; int status;
	...
	switch (event) {
		case EVENT_COMMIT:
			 ...
			 else if(control==TEL_IN || control==TEL_OUT) focusadjust();
			 else if(control==TEL_INTICK || control==TEL_OUTTICK) focus_tick(control);
			 
	         else if(control==TEL_GOTO) goto_target();
			 
 	         else if(control==TEL_TOGGLE)
               {GetCtrlVal(tel_handle,TEL_TOGGLE,&status);
			    if(status==1) set_offset();
				else
                  {SetCtrlVal(tel_handle,TEL_OFFRA,0.0); SetCtrlVal(tel_handle,TEL_OFFDEC,0.0);}
               }
			 
 	         else if(control==TEL_TOGGLE2)
               {GetCtrlVal(tel_handle,TEL_TOGGLE2,&status);
			    if(status==1) set_offset2();
				else
                  {SetCtrlVal(tel_handle,TEL_OFFRA,0.0); SetCtrlVal(tel_handle,TEL_OFFDEC,0.0);}
               }
...
//              check dome position                      
             ...
             else if(TELESCOPE_MODE == TEL_MODE_SLEW ) check_slew();
...
//-----------------------------------------------------------------------------
{
 int state1,state2;
	...
	GetCtrlVal(tel_handle,TEL_IN ,&state1);
	GetCtrlVal(tel_handle,TEL_OUT,&state2);
	if(state1==0 && state2==0)  adjuat_focus(0); 
	else if(state1==1)  {adjuat_focus( 1); SetCtrlVal(tel_handle,TEL_OUT,0);}
	else if(state2==1)  {adjuat_focus(-1); SetCtrlVal(tel_handle,TEL_IN ,0);} 
...
//-----------------------------------------------------------------------------
{
 ...
 double time;
	...
	GetCtrlVal(tel_handle,TEL_TICKTIME,&time);
	if     (control==TEL_INTICK) 
	  {SetCtrlVal(tel_handle,TEL_IN,1);adjuat_focus( 1); Delay(time);adjuat_focus(0);SetCtrlVal(tel_handle,TEL_IN,0);}
	else if(control==TEL_OUTTICK)
	  {SetCtrlVal(tel_handle,TEL_OUT,1);adjuat_focus(-1); Delay(time);adjuat_focus(0);SetCtrlVal(tel_handle,TEL_OUT,0);}
