...
//  claculate frequency from speed
     SLEW_FREQ[0] =  SLEW_SPEED[0] *3600.0* MOUNT_HA[1];  // in arcminute/sec
    GUIDE_FREQ[0] = GUIDE_SPEED[0] * 60.0 * MOUNT_HA[1];  // in arcminute/sec
     GSET_FREQ[0] =  GSET_SPEED[0] * 60.0 * MOUNT_HA[1];  // in arcminute/sec
     INIT_FREQ[0] =  INIT_SPEED[0]        * MOUNT_HA[1];  // arcsec / sec

     SLEW_FREQ[1] =  SLEW_SPEED[1] *3600.0* MOUNT_DEC[1];  // in arcminute/sec
    GUIDE_FREQ[1] = GUIDE_SPEED[1] * 60.0 * MOUNT_DEC[1]; // in arcminute/sec
     GSET_FREQ[1] =  GSET_SPEED[1] * 60.0 * MOUNT_DEC[1]; // in arcminute/sec
...
//------------------------------------------------------------------------
{
 ...
 long   azi0,alt0,der0,step1,step2,lRef1,lRef2;
...
//      Altitude                       
        ...
        if( rad_to_deg(alt.rad) > 85)
          {Beep(); 
           if(ConfirmPopup("고도가 너무 높습니다","돌아갈까요 ?")==1)
             {
              STOP_TELESCOPE();
              return -1;
             }
